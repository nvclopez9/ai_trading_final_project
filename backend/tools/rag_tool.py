...
Flujo del RAG en este proyecto:
  1. Offline (una vez): ``python -m backend.rag.ingest`` lee los PDFs de
...
def _get_vectorstore():
    ...
    if not persist_path.exists() or not any(persist_path.iterdir()):
        _init_error = "Base de conocimiento no inicializada. Ejecuta `python -m backend.rag.ingest` primero."
        return None
    ...
    try:
        ...
        try:
            if _vectorstore._collection.count() == 0:
                _init_error = "Base de conocimiento vacía. Ejecuta `python -m backend.rag.ingest` primero."
                _vectorstore = None
                return None
        except Exception:
            pass
