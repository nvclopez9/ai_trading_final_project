...
Con modelos pequeños el tool-calling es más fiable cuando la observación ya
es texto plano que el LLM puede resumir sin tener que serializar/parsear JSON.
Además, cualquier excepción se captura y se convierte en un mensaje legible;
así el agente nunca recibe un stacktrace que rompa su bucle de razonamiento.
"""
...
from backend.tools.universes import (
    LARGE_CAP,
    MID_CAP,
    ETFS,
    ETPS_COMMODITIES,
...
def _fetch_fallback_quotes():
    ...
    serie (no paralelizada) para mantener el código simple; con 30
